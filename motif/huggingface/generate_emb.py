...
def main(cfg: DictConfig) -> float:
    ...
    with torch.no_grad():
        for batch in tqdm(data_loader):
            ...
            for sampleid in range(len(batch["atu"])):
                new_df.append({
                    "atu": batch["atu"][sampleid],
                    "desc": batch["desc"][sampleid],
                    "text": batch["text"][sampleid],
                    name: emb[sampleid]
                })
    
    df = pd.DataFrame(new_df)
    atu = pd.read_hdf(cfg.data.h5_file, key=cfg.data.h5_key)
    merged = pd.merge(df, atu, how='inner',
            on=["atu", "desc", "text"])
    assert atu.shape[0] == merged.shape[0], "row should be the same"
    print(merged.columns)
    merged.to_hdf(cfg.data.h5_file, key=cfg.data.h5_key)
